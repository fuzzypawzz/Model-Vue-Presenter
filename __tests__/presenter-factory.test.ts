...
describe('presenter factory behaviour', () => {
    ...
    it.each([
        ...
        {},
        null,
    ])('throws if the returned viewModel is not a computed property', (viewModel) => {
...
describe('view model mocking', () => {
    it('returns the mocked view model', () => {
        ...
        expect(viewModel.value.isProductsLoading).toBe(testHarness.mockedViewModelValues.isProductsLoading)
    })
    ...
    it('mocks the view model for the next resolved presenter instance only', () => {
        ...
        expect(firstInstance.viewModel.value.products.length).toBe(testHarness.mockedViewModelValues.products.length)
        expect(firstInstance.viewModel.value.isProductsLoading).toBe(testHarness.mockedViewModelValues.isProductsLoading)
        ...
        expect(secondInstance.viewModel.value.products.length).toBe(testHarness.actualViewModelValues.products.length)
        expect(secondInstance.viewModel.value.isProductsLoading).toBe(testHarness.actualViewModelValues.isProductsLoading)
    })
    ...
    it('throws if the mocked view model contains properties that is not available in the actual view model', () => {
        ...
        const expectedError = ERROR.VIEW_MODEL_MOCKING.HAS_NON_EXISTING_PROPERTIES(Object.keys(nonExistingProperties))
